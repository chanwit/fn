...
if [ -z $(grep -m1 -Eo "[0-9]+\.[0-9]+\.[0-9]+-ske" $version_file) ]; then
  echo "did not find semantic version in $version_file"
  exit 1
fi
perl -i -pe 's/\d+\.\d+\.\K(\d+)/$1+1/e' $version_file
version=$(grep -m1 -Eo "[0-9]+\.[0-9]+\.[0-9]+-ske" $version_file)
...
docker tag fnproject/$image:latest $user/$image:$version
